  ...
  return (
    <>
      ...
      <section style={{minHeight:'100vh',position:'relative',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1,textAlign:'center',maxWidth:'900px',padding:'0 2rem',marginTop:'5rem'}}>
          ...
          <h1 style={{fontSize:'clamp(2.5rem,6vw,5rem)',fontWeight:400,lineHeight:1.15,marginBottom:'1.5rem',color:'#ffffff'}}>
            ...
            <span style={{background:'linear-gradient(135deg,#a38057 0%,#cda882 50%,#a38057 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',fontStyle:'italic'}}>Luxury</span>
          </h1>
            ...
            {db.rooms.slice(0,3).map(room=>(
              <div key={room.id} className="card-hover" style={{background:'#162032',border:'1px solid rgba(163,128,87,0.15)',overflow:'hidden'}}>
                <div style={{position:'relative',height:'220px'}}>
                  ...
                  <div style={{position:'absolute',top:'1rem',right:'1rem',background:room.available?'rgba(163,128,87,0.9)':'rgba(139,90,90,0.9)',color:'#0d1b2a',fontSize:'0.65rem',padding:'0.25rem 0.7rem',fontFamily:'var(--font-sans)',fontWeight:700,letterSpacing:'0.1em'}}>
                    {room.available?'AVAILABLE':'BOOKED'}
                  </div>
                </div>
                <div style={{padding:'1.5rem'}}>
                  <h3 style={{color:'#cda882',fontSize:'1.15rem',fontWeight:400,marginBottom:'0.5rem'}}>{room.name}</h3>
                  ...
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'0.75rem'}}>
                    <div>
                      <span style={{color:'#b8956b',fontSize:'1.4rem',fontWeight:700,fontFamily:'var(--font-sans)'}}>PKR {room.price.toLocaleString()}</span>
      ...
      <section className="section-pad" style={{background:'#0a1520'}}>
        <div style={{maxWidth:'1280px',margin:'0 auto',padding:'0 2rem'}}>
          <div style={{textAlign:'center',marginBottom:'3.5rem'}}>
            <p style={{color:'#a38057',fontSize:'0.7rem',letterSpacing:'0.3em',fontFamily:'var(--font-sans)',textTransform:'uppercase',marginBottom:'0.75rem'}}>What We Offer</p>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:'1.5rem'}}>
            {db.amenities.map((a, i) => {
              ...
              return (
                <div key={i} style={{background:'rgba(22,32,50,0.8)',border:'1px solid rgba(163,128,87,0.12)',padding:'2rem 1.5rem',textAlign:'center',transition:'all 0.3s',cursor:'default'}}
                  onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(163,128,87,0.4)';(e.currentTarget as HTMLElement).style.background='rgba(163,128,87,0.06)';}}
                  onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(163,128,87,0.12)';(e.currentTarget as HTMLElement).style.background='rgba(22,32,50,0.8)';}}
                >
                  <div style={{display:'flex',justifyContent:'center',marginBottom:'0.75rem'}}>
                    {AmenityIcon && <AmenityIcon size={32} color="#a38057" />}
                  </div>
      ...
      <section className="section-pad" style={{background:'#0d1b2a'}}>
        <div style={{maxWidth:'1100px',margin:'0 auto',padding:'0 2rem'}}>
          <div style={{textAlign:'center',marginBottom:'3.5rem'}}>
            <p style={{color:'#a38057',fontSize:'0.7rem',letterSpacing:'0.3em',fontFamily:'var(--font-sans)',textTransform:'uppercase',marginBottom:'0.75rem'}}>Guest Stories</p>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,300px),1fr))',gap:'2rem'}}>
            {db.testimonials.map(t=>(
              <div key={t.id} style={{background:'#162032',border:'1px solid rgba(163,128,87,0.15)',padding:'2rem'}}>
                <div style={{color:'#a38057',fontSize:'3rem',lineHeight:1,fontFamily:'var(--font-serif)',marginBottom:'1rem',opacity:0.6}}>"</div>
